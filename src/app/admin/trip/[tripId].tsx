...
const TripDetailsScreen = () => {
  ...
  <View className="bg-white rounded-lg p-4 mb-6 ">
    ...
    {[
      ...
      { label: "Truck Plate Number", value: tripInfo?.plate_number },
      { label: "Truck Model", value: tripInfo?.truck_model },
      { label: "Customer Name", value: tripInfo?.customer_name },
    ]
      .filter(item => item.value) // This filters out items with empty or undefined values
      .map((item, index) => (
        <View
          key={index}
          className="flex-row justify-between mb-1 border-b-[1px] border-[#F0F1F1] py-3"
  ...
  <View className="bg-white rounded-lg p-4 mb-6 ">
    {tripInfo?.vendor_name && (<View className="border-b-[1px] border-[#F0F1F1]">
