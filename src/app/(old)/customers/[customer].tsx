...
const TruckDriverInfo = () => {
  ...
  const{driver:customerId} = useLocalSearchParams()
  ...
  return (
    <View className="flex-1 p-6">
      <View className="bg-white rounded-lg p-3">
        <Image
          source={{uri:customerData?.image || fallbackDriverImage}}
          className="w-24 h-24 rounded-full mx-auto mb-4 items-center"
        />
        {[
          { label: "Name", value: customerData?.first_name + customerData?.last_name },
