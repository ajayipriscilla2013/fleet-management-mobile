...
const TripDetailsScreen = () => {
  ...
  const { data: tripInfo,isLoading } = useQuery({
    queryKey: ["TripInfoForDriver",tripId],
          ...
          if (tripInfo.fuelling === 1 && tripInfo.fueled=== 0) {
            return (
              ...
              <TouchableOpacity
               className="bg-[#394F91] rounded-2xl p-4 mt-6"
              onPress={() =>
                   handlePress(`/driver/trip/confirm-loading-point-details/${tripId}`)
                 }
             >
               <Text className="text-white text-center font-semibold">
                  Accept Trip
        ...
        if (tripInfo.driver_loading_confirmed === 1 && tripInfo.driver_offloading_confirmed=== 1) {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() =>
                handlePress(
                  `/driver/trip/confirm-offloading-point-details/${tripId}?destination=${tripInfo?.destination_name}`
                )
              }
            >
              <Text className="text-white text-center font-semibold">
